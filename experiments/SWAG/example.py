...
class SWAGInference(object):
    ...
    def __init__(
        ...
        swag_epochs: int = 2,
        swag_learning_rate: float = 0.035,
    ...
    def update_swag(self) -> None:
        ...
        if self.inference_mode == InferenceMode.SWAG_FULL:
            print(f"(updating D for n={self.n}), len(self.D)={len(self.D)}")
            ...
            for name, param in current_params.items():
                theta_difference[name] = torch.clamp(
                    param - self.theta[name], min=1e-10)
    ...
    def fit_swag(self, loader: torch.utils.data.DataLoader) -> None:
        ...
        # QUESTION: I think this might be "wrong" because the constructor
    ...
    def predict_probabilities_swag(self, loader: torch.utils.data.DataLoader) -> torch.Tensor:
        ...
        for _ in tqdm.trange(self.bma_samples, desc="Performing Bayesian model averaging"):
            ...
            breakpoint()
            predictions = torch.softmax(torch.cat(predictions), dim=-1)
        ...
        # We add all model predictions together and take the mean
