...
def evaluate_12ECG_score(label_directory, output_directory):
    ...
    weights_file = 'reward_matrix_weights.csv'
    normal_class = '426783006'  # Class for normal sinus rythm i.e. healthy
    equivalent_classes = [
        ['713427006', '59118001'],
        ['284470004', '63593006'],
        ['427172004', '17338001']
    ...
    label_classes, labels = load_labels(
        label_files, normal_class, equivalent_classes)
    ...
    # Find indices of classes in weight matrix.
    ...
    for i in range(num_recordings):
        with open(output_files[i], 'r') as f:
            ...
            if len(lines) >= 3 and len(set(lengths)) == 1:
                ...
                print(
                    '- The output file {} has formatting errors, so all outputs are assumed to be negative for this recording.'.format(output_files[i]))
...
def organize_labels_outputs(label_classes, output_classes, tmp_labels, tmp_binary_outputs, tmp_scalar_outputs):
    ...
    assert (len(tmp_labels) == len(tmp_binary_outputs)
            == len(tmp_scalar_outputs))
        ...
        for i in range(num_recordings):
            for j in range(num_classes):
                if labels[i, j] == 1 and outputs[i, j] == 1:  # TP
                    ...
                    breakpoint()
                    raise ValueError(
                        'Error in computing the confusion matrix.')
    else:
        A = np.zeros((num_classes, 2, 2))
        for i in range(num_recordings):
...
def compute_beta_measures(labels, outputs, beta):
    ...
    for k in range(num_classes):
        ...
        if (1+beta**2)*tp + fp + beta**2*fn:
            f_beta_measure[k] = float(
                (1+beta**2)*tp) / float((1+beta**2)*tp + fp + beta**2*fn)
        else:
...
def compute_challenge_metric(weights, labels, outputs, classes, normal_class):
    ...
    if correct_score != inactive_score:
        normalized_score = float(
            observed_score - inactive_score) / float(correct_score - inactive_score)
    else:
